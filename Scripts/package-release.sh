...
rm -f "$OUTPUT_DIR/$APP_ARCHIVE_NAME" "$OUTPUT_DIR/$CLI_ARCHIVE_NAME" "$OUTPUT_DIR/$CHECKSUM_FILE_NAME"
...
(
  cd "$OUTPUT_DIR"
  shasum -a 256 "$APP_ARCHIVE_NAME" "$CLI_ARCHIVE_NAME" > "$CHECKSUM_FILE_NAME"
)
